...
public class DetailRunMultiSymbols {
    ...
    public static void run(TradingAlgorithm algorithm, String stockList, Float gap, Float splitRatioPower) {
        ...
        for (int index = 0; index < tradeGroupSize; index++) {
            ...
            runJustOne(tradings, index, gap, lossMajor, includeSummary, anyDayGap);
        }
    }
    private static void runJustOne(List<Trading> tradings, int index, Float gap, boolean lossMajor, boolean includeSummary, boolean anyDayGap) {
        String num = "_" + String.format("%1d", index) + "_";
        GroupTradeDayGapRatioTrading g = TradingAlgorithm.getAlgorithm(algorithm);
        g.setGapSize(gap);
    ...
    private static void creatSymbolList(GroupTradeDayGapRatioTrading g, List<GroupTradeResult> results) {
